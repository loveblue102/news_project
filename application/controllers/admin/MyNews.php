			...
			foreach ($session["per"] as $value) {
				if ($value["action_code"] == "my_news") {
					...
					if (in_array(2, $check) || in_array(9, $check)) {
						...
						$formAction = base_url() . "admin/MyNews/DoAddNews";
...
//						die();
						header('location:' . base_url() . 'admin/MyNews/ListNews');
						//end khung
					}
				}
			}
			...
			foreach ($session["per"] as $value) {
				if ($value["action_code"] == "my_news") {
					...
					if (in_array(1, $check) || in_array(9, $check)) {
						...
						$config['total_rows'] = $this->NewsModel->numMyNews($categoryId,$user_id);//tat ca ban ghi
//						echo $config['total_rows'];
						...
						$config['next_link'] = 'Next »';
						$config['prev_link'] = '« Prev';
						$config['base_url'] = base_url() . 'admin/MyNews/Fillter/'.$categoryId;
						$config['uri_segment'] = 5;
						$this->pagination->initialize($config);
						$offset = ($this->uri->segment(5) == '') ? 0 : ($this->uri->segment(5) - 1)* $config['per_page'];
//						$offset = $offset ;
						$data = $this->NewsModel->pagiNews($user_id,$categoryId, $config['per_page'], $offset);
						$data = $this->CategoryNews($data);
						$category = $this->Layout();
						view('admin.MyNews', compact('data', 'category','session'));
			...
			foreach ($session["per"] as $value) {
				if ($value["action_code"] == "my_news") {
					...
					if (in_array(1, $check) || in_array(9, $check)) {
						...
						view('admin.MyNewsDetails',compact('PostDetails','session'));
						//end khung
					}
				}
			}
			...
			foreach ($session["per"] as $value) {
				if ($value["action_code"] == "my_news") {
					...
					if (in_array(3, $check) || in_array(9, $check)) {
						...
						$status = $this->NewsModel->getNewsStatus('news_status');
						$kind = $this->NewsModel->getNewsStatus('news_kind');
						$formAction = base_url() . "admin/MyNews/DoEdit/".$news_id;
