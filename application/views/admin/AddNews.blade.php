		...
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				<form method="post" action="{{ $formAction }}" enctype="multipart/form-data">
					...
					@if(isset($data) && $session["per_name"] != "mod")
					<div class="row" style="margin:5px">
						<div class="col-md-2">Trang Thai</div>
						<div class="col-md-10">
							<select name="status" id="" style="width: 114px">
								@foreach($status as $value)
									<option <?= isset($value["news_status"]) && $value["news_status"] == $data["news_status"]?"selected":""; ?> value="{{ $value["news_status"] }}">{{ $value["news_status"] }}</option>
								@endforeach
							</select>
						</div>
					</div>
