					...
					<li class="dropdown user user-menu">
						<a href="#" class="dropdown-toggle" data-toggle="dropdown">
							<img src="<?= base_url() ?>asset/upload/<?= isset($session["user_image"])? $session["user_image"] : "" ?>" class="user-image"
								 alt="User Image">
							<span class="hidden-xs">{{ isset($session["username"])? $session["username"] : ""}}</span>
						</a>
						<ul class="dropdown-menu">
							<!-- UserModel image -->
							<li class="user-header">
								...
								<p>
									...
									<small>{{ isset($session["per_name"])? $session["per_name"] : ""}}</small>
								</p>
							</li>
							<!-- Menu Footer-->
	...
	<aside class="main-sidebar">
		...
		<section class="sidebar">
			...
			<div class="user-panel">
				...
				<div class="pull-left info">
					<p>{{ isset($session["username"])? $session["username"] : ""}}</p>
					<!-- Status -->
		...
		@foreach($session["per"] as $value)
			<!-- Sidebar Menu -->
				...
				?>
				@if( $value["action_code"] == "my_news" && (in_array(1, $check) || in_array(2, $check) || in_array(9, $check)) )
					<li class="treeview">
						<a href="#">
				...
				@endif
					...
					@if( $value["action_code"] == "news" && (in_array(1, $check) || in_array(2, $check) || in_array(9, $check)) )
						<li class="treeview">
							<a href="#">
								<i class="fa fa-files-o"></i>
								<span>Tin Tức</span>
					...
					@endif
					@if( $value["action_code"] == "acount" && (in_array(1, $check) || in_array(2, $check) || in_array(9, $check)) )
						<li class="treeview">
							<a href="#">
								<i class="fa fa-th"></i>
								<span>Tài Khoản</span>
      ...
      <i class="fa fa-angle-left pull-right"></i>
					...
					@endif
					@if( $value["action_code"] == "category_news" && (in_array(1, $check) || in_array(2, $check) || in_array(9, $check)) )
						<li class="treeview">
							<a href="#">
