		...
		<div class="col-md-2 " style="margin-left: 14px !important;">
			@if(isset($session))
				@foreach($session["per"] as $row)
					...
					?>
					@if($row["action_code"] == "my_news" && (in_array(2, $check) || in_array(9, $check)))
							...
							<td style="text-align:center">
								@if(isset($session))
									@foreach($session["per"] as $row)
										...
										$check = explode(',', $row["check_action"]);
										?>
										@if($row["action_code"] == "my_news" && (in_array(3, $check) || in_array(9, $check)))
											<a href="<?= base_url() ?>admin/MyNews/Edit/<?= $value["news_id"]?>">Edit</a>
										@endif
										@if($row["action_code"] == "my_news" && (in_array(4, $check) || in_array(9, $check)))
