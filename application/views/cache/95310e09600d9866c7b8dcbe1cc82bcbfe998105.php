<?php /* C:\xampp\htdocs\ows_news\application\views/admin/MyNews.blade.php */ ?>
		...
		<div class="col-md-2 " style="margin-left: 14px !important;">
			<?php if(isset($session)): ?>
				<?php $__currentLoopData = $session["per"]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
					...
					?>
					<?php if($row["action_code"] == "my_news" && (in_array(2, $check) || in_array(9, $check))): ?>
							<a href="<?= base_url() ?>admin/MyNews/AddNews" class="btn btn-primary">
								Add news
							</a>
					<?php endif; ?>
				...
				<table class="table table-bordered table-hover">
					...
					<?php $__currentLoopData = $data; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
						<tr>
							...
							<td>
								<a href="<?= base_url()?>admin/MyNews/NewsDetails/<?= $value["news_id"]?>">
							...
							<td style="text-align:center">
								<?php if(isset($session)): ?>
									<?php $__currentLoopData = $session["per"]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
										...
										$check = explode(',', $row["check_action"]);
										?>
										<?php if($row["action_code"] == "my_news" && (in_array(3, $check) || in_array(9, $check))): ?>
											<a href="<?= base_url() ?>admin/MyNews/Edit/<?= $value["news_id"]?>">Edit</a>
										<?php endif; ?>
										<?php if($row["action_code"] == "my_news" && (in_array(4, $check) || in_array(9, $check))): ?>
											<a href="<?= base_url() ?>admin/MyNews/Delete/<?= $value["news_id"]?>"
